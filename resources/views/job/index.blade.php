...
@section('title', 'Job Applications & Referral')
@section('header_title', 'Job Placement & Referral Module')

@section('content')
<!-- Program Header Banner -->
<div style="background: linear-gradient(135deg, #A0031E, var(--peso-red)); color: #FFFFFF; padding: 24px; border-radius: var(--radius-lg); margin-bottom: 24px; display: flex; align-items: center; justify-content: space-between;">
    <div>
        <div style="display: flex; align-items: center; gap: 10px;">
            <span class="badge badge-red" style="font-size: 14px; padding: 6px 12px; background: #FFFFFF; color: var(--peso-red-dark);">JOB MODULE</span>
            <h2 style="font-size: 22px; font-weight: 800; color: #FFFFFF;">Job Placement & Referral Program</h2>
        </div>
        <p style="font-size: 13px; color: #FEE2E2; margin-top: 6px;">
            Connects jobseekers in Agoo, La Union with local, regional, and national employer vacancies.
        </p>
    </div>
    <div style="background: rgba(255,255,255,0.15); padding: 12px 20px; border-radius: var(--radius-md); text-align: center;">
        <span style="font-size: 11px; text-transform: uppercase; color: var(--peso-yellow); font-weight: 700;">Total Job Applications</span>
...
<div class="panel-card">
    <div class="panel-header">
        <div class="panel-title">
            <i data-lucide="building-2" style="color: var(--peso-red);"></i>
            <h3>Job Referral Applications Directory</h3>
        </div>
    </div>

    <!-- Search & Filter Controls -->
    <div style="padding: 16px 24px; background: #F8FAFC; border-bottom: 1px solid var(--border-color);">
        <form action="{{ route('admin.job.index') }}" method="GET" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
            <div style="flex-grow: 1; min-width: 200px;">
                <input type="text" name="search" class="form-control" placeholder="Search applicant, desired position, company..." value="{{ request('search') }}">
            </div>
            ...
            @if(request('search') || request('status') || request('date_from') || request('date_to'))
                <a href="{{ route('admin.job.index') }}" class="btn btn-secondary" style="color: var(--peso-red);">Reset</a>
            @endif
        </form>
    </div>
            ...
            <tbody>
                @forelse($applications as $app)
                    <tr>
                        ...
                        <td>{{ $app->place_or_agency ?? 'Local Employer' }}</td>
                            ...
                            <div class="data-action-stack">
                                ...
                                <div class="action-button-group">
                                    ...
                                    <form action="{{ route('admin.applications.destroy', $app->id) }}" method="POST"
                                          onsubmit="return confirm('Are you sure you want to delete this Job application record? This action cannot be undone.');">
                                        @csrf
                                        @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="8" style="text-align: center; color: var(--text-muted); padding: 30px;">
                            No Job placement applications recorded.
                        </td>
                    </tr>
                @endforelse
            </tbody>
