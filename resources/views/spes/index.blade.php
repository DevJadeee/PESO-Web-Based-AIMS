...
@section('title', 'SPES Applications')
@section('header_title', 'Special Program for Employment of Students (SPES)')

@section('content')
<!-- Program Header Banner -->
<div style="background: linear-gradient(135deg, #065F46, var(--peso-green)); color: #FFFFFF; padding: 24px; border-radius: var(--radius-lg); margin-bottom: 24px; display: flex; align-items: center; justify-content: space-between;">
    <div>
        <div style="display: flex; align-items: center; gap: 10px;">
            <span class="badge badge-green" style="font-size: 14px; padding: 6px 12px; background: #FFFFFF; color: var(--peso-green);">SPES MODULE</span>
            <h2 style="font-size: 22px; font-weight: 800; color: #FFFFFF;">Special Program for Employment of Students</h2>
        </div>
        <p style="font-size: 13px; color: #D1FAE5; margin-top: 6px;">
            Assists deserving students and out-of-school youth in pursuing education through employment support.
        </p>
    </div>
    <div style="background: rgba(255,255,255,0.15); padding: 12px 20px; border-radius: var(--radius-md); text-align: center;">
        <span style="font-size: 11px; text-transform: uppercase; color: var(--peso-yellow); font-weight: 700;">Total SPES Applications</span>
...
<div class="panel-card">
    <div class="panel-header">
        <div class="panel-title">
            <i data-lucide="graduation-cap" style="color: var(--peso-green);"></i>
            <h3>SPES Student Applications Directory</h3>
        </div>
    </div>

    <!-- Search & Filter Controls -->
    <div style="padding: 16px 24px; background: #F8FAFC; border-bottom: 1px solid var(--border-color);">
        <form action="{{ route('admin.spes.index') }}" method="GET" style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center;">
            <div style="flex-grow: 1; min-width: 200px;">
                <input type="text" name="search" class="form-control" placeholder="Search student name, school, purpose..." value="{{ request('search') }}">
            </div>
            ...
            @if(request('search') || request('status') || request('date_from') || request('date_to'))
                <a href="{{ route('admin.spes.index') }}" class="btn btn-secondary" style="color: var(--peso-red);">Reset</a>
            @endif
        </form>
    </div>
    ...
    <div class="table-responsive">
        <table class="peso-table">
            <thead>
                <tr>
                    <th>Date</th>
                    <th>Student Applicant Name</th>
                    <th>Place / Department</th>
            ...
            <tbody>
                @forelse($applications as $app)
                    <tr>
                        ...
                        <td>{{ $app->place_or_agency ?? 'Municipal Office' }}</td>
                            ...
                            <div class="data-action-stack">
                                ...
                                <div class="action-button-group">
                                    ...
                                    <form action="{{ route('admin.applications.destroy', $app->id) }}" method="POST"
                                          onsubmit="return confirm('Are you sure you want to delete this SPES application record? This action cannot be undone.');">
                                        @csrf
                                        @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="8" style="text-align: center; color: var(--text-muted); padding: 30px;">
                            No SPES student applications recorded.
                        </td>
                    </tr>
                @endforelse
            </tbody>
